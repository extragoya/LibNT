...
%Given two MIAExpr's will pull how to permute B to get its indices to match
%up with A. E.g., if a_indices is 'ijk' and b_indices is 'jki', will
%produce array [3 1 2].
if ~isa(B,'MIAExpr') 
    error('MIAExpr''s can only be merged with other MIAExpr''s');
end
a_order=A.m_mia.order;
b_order=B.m_mia.order;
if(a_order~=b_order)
    error('Only MIAs of the same order can be added');
end

a_indices=A.m_indices;
b_indices=B.m_indices;

reg='[a-z]'; %only interested in matching letter indices, ignoring any exclamation marks
