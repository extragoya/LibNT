...
if length(a_inner_idx)~=length(b_inner_idx)
    error('Number of indices to undergo an inner product must be the same for each NT');
end


if length(a_inter_idx)~=length(b_inter_idx)
    error('Number of indices to undergo an inter product must be the same for each NT');
end
