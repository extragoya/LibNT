...
inline void _RadixSort_Unsigned_PowerOf2Radix_L1( ForwardIt begin, FollowIt followBegin, size_t length, unsigned long shiftRightAmount )
{
    ...
    {
        unsigned long digit;
        auto _current_element_it = begin+_current; // get the compiler to recognize that a register can be used for the loop instead of a[_current] memory location
        auto _current_element_follow_it = followBegin+_current; // get the compiler to recognize that a register can be used for the loop instead of a[_current] memory location
        while( endOfBin[ digit = (unsigned long)((*_current_element_it)>>shiftRightAmount& BitMask)] != _current ){
            std::iter_swap( _current_element_it, begin+ endOfBin[ digit ]  );
            std::iter_swap( _current_element_follow_it,followBegin+ endOfBin[ digit ]++  );
        }
	...
	inline float estimateNumberOfRuns(size_t & length, size_t max_size) {

		float numberOfRuns = 0;
		int radixRuns = static_cast<int>(std::ceil(std::log2(max_size) / std::log2(Radix)));
		float first_run =float(max_size) / static_cast<size_t>(std::pow(Radix, radixRuns - 1));
	...
	public:
		...
		*/
		...
		inline float estimateNumberOfRunsShuffle(size_t t_length, int start_index = 0) {
			...
			for (int i = start_index; i < mMaxDims.size(); i += 2) {
				...
				if (i + 1 < mMaxDims.size()) {
					...
					num_runs += .5; //penalise a find operation by 0.5
					/*if (t_length <= InsertThreshold)
						num_runs += .5; */
				}
			}
		...
		void permute(RandomIt begin, FollowIt  followBegin, size_t length)
		{
			...
			float num_straight_runs = estimateNumberOfRuns<Threshold, InsertThreshold, PowerOfTwoRadix>(t_length, mTotalMax);
			if (mFirstSortOrFind == false)  //if we are currently finding and not sorting
			{

				if (mMaxDims.size() <= 1)
					return;
				float num_runs = estimateNumberOfRunsShuffle(length,1)+0.5;
			...
			else
			{
				...
				float num_runs = estimateNumberOfRunsShuffle(length);
