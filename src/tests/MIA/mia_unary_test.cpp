...
void dense_unary_work(){
    ...
    BOOST_CHECK_MESSAGE(b==other_b,std::string("Simple Contraction 1 for ")+typeid(data_type).name());

    a=LibMIA::DenseMIA<data_type,6>(_dim,_dim+1,_dim,_dim+2,_dim-1,_dim-2); //now try with non-uniform dimensions
    a.randu(-5,5);
    b(j,k,l,m)=a(i,j,i,k,l,m); //contraction
    other_b(j,k,l,m)=a(i,j,n,k,l,m)*delta(i,n); //equivalent operation
    BOOST_CHECK_MESSAGE(b==other_b,std::string("Simple Contraction 2 for ")+typeid(data_type).name());
    ...
    BOOST_CHECK_MESSAGE(c==other_c,std::string("Triple Contraction 1 for ")+typeid(data_type).name());
    ...
    BOOST_CHECK_MESSAGE(d==other_d,std::string("Complex Contraction 1 for ")+typeid(data_type).name());
    ...
    BOOST_CHECK_MESSAGE(e==other_e,std::string("Complex Contraction 2 for ")+typeid(data_type).name());

    d=LibMIA::DenseMIA<data_type,2>(_dim,_dim);
    d.randu(-5,5);
    e(i)=d(!i,!i);
    other_e(i)=d(!i,j)*delta(!i,j);
    BOOST_CHECK_MESSAGE(e==other_e,std::string("Very Simple Attraction 1 for ")+typeid(data_type).name());
    ...
    BOOST_CHECK_MESSAGE(f==other_f,std::string("Simple Attraction 1 for ")+typeid(data_type).name());
    ...
    BOOST_CHECK_MESSAGE(f==other_f,std::string("Simple Attraction 2 for ")+typeid(data_type).name());
    ...
    BOOST_CHECK_MESSAGE(b==other_b,std::string("Triple Attraction 1 for ")+typeid(data_type).name());
    ...
    BOOST_CHECK_MESSAGE(b==other_b,std::string("Complex Attraction 1 for ")+typeid(data_type).name());

    a=LibMIA::DenseMIA<data_type,6>(_dim,_dim+1,_dim,_dim+1,_dim,_dim-2); //now try with non-uniform dimensions
    a.randu(-5,5);
    c(i,j,m)=a(!i,!j,!i,!j,!i,m); //attraction
    other_c(i,j,m)=a(!i,!j,n,k,l,m)*delta_3(!i,n,l)*delta2(!j,k); //equivalent operation
    BOOST_CHECK_MESSAGE(c==other_c,std::string("Complex Attraction 2 for ")+typeid(data_type).name());
    ...
    BOOST_CHECK_MESSAGE(d==other_d,std::string("Complex Attraction 3 for ")+typeid(data_type).name());

    a=LibMIA::DenseMIA<data_type,6>(_dim,_dim+1,_dim,_dim+1,_dim,_dim-2); //now try with non-uniform dimensions
    a.randu(-5,5);
    c(i,k,m)=a(!i,j,!i,j,k,m); //attraction
    other_c(i,k,m)=a(n,l,!i,j,k,m)*delta(!i,n)*delta2(j,l); //attraction
    BOOST_CHECK_MESSAGE(c==other_c,std::string("Combined Contraction/Attraction 1 for ")+typeid(data_type).name());

    b=LibMIA::DenseMIA<data_type,4>(_dim,_dim+1,_dim,_dim+1); //now try with non-uniform dimensions
    b.randu(-5,5);
    e(i)=b(!i,j,!i,j); //attraction
    other_e(i)=b(n,l,!i,j)*delta(!i,n)*delta2(j,l); //attraction
    BOOST_CHECK_MESSAGE(e==other_e,std::string("Combined Contraction/Attraction 2 for ")+typeid(data_type).name());
    ...
    BOOST_CHECK_MESSAGE(d==other_d,std::string("Combined Contraction/Attraction 3 for ")+typeid(data_type).name());

    a=LibMIA::DenseMIA<data_type,6>(_dim,_dim+1,_dim,_dim,_dim+1,_dim-2); //now try with non-uniform dimensions
    a.randu(-5,5);
    d(j,m)=a(i,!j,i,i,!j,m); //attraction
    other_d(j,m)=a(i,!j,n,k,l,m)*delta_3(i,n,k)*delta2(!j,l); //attraction
    BOOST_CHECK_MESSAGE(d==other_d,std::string("Combined Contraction/Attraction 4 for ")+typeid(data_type).name());
    ...
    BOOST_CHECK_MESSAGE(d==other_d,std::string("Combined Contraction/Attraction 5 for ")+typeid(data_type).name());
